...
- 悬浮球快捷面板：沿用 PicBoard 的小球动画，随时查看今日 Token、预估花费并点击复制历史记录
...
悬浮球默认显示。单击打开面板，长按约 250 毫秒或直接拖动即可移动位置；点击收起按钮或按 `Esc` 回到小球。面板中的文本、图片和文件记录均可点击复制，“全部记录”会打开完整剪贴板页面。

悬浮面板跟随仪表板的主题和货币设置，今日用量按本地日期统计。托盘菜单可以显示、隐藏悬浮球或重置位置；位置及显示状态保存在 `%LOCALAPPDATA%\TokenTracker\floating-window.json`，重启后恢复。剪贴板仍使用原来的本地持久化记录。
