...
export function SkillDetailPanel({
  ...
  // Keep the last skill mounted until Base UI finishes its close transition.
  // Clearing the parent selection immediately removes Dialog.Popup before the
  // transition manager can observe the ending state, leaving a transparent
  // backdrop mounted forever and blocking every subsequent click.
  ...
  const activeSkill = skill || displaySkill;
  ...
  return (
    <Dialog.Root
      ...
      onOpenChangeComplete={(nextOpen) => {
        if (!nextOpen) setDisplaySkill(null);
      }}
    >
      <Dialog.Portal>
        <Dialog.Backdrop className="fixed inset-0 z-[80] bg-oai-black/40 backdrop-blur-[2px] transition-opacity duration-200 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0" />
        <Dialog.Viewport
          data-skill-detail-viewport="1"
          className="fixed inset-0 z-[81] flex items-center justify-center overflow-y-auto p-4 sm:p-6"
        >
          {activeSkill ? (
            <SkillDetailPanelInner
              key={activeSkill.id || activeSkill.directory}
              skill={activeSkill}
              targets={targets}
              busyKey={busyKey}
              usage={usage}
...
function SkillDetailPanelInner({
  ...
  const hasUsage = Boolean(usage?.invocations);

  return (
    <Dialog.Popup
      data-skill-detail-modal="1"
      className="relative my-auto flex max-h-[calc(100vh-2rem)] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-white/50 bg-white/90 shadow-[0_28px_80px_-24px_rgba(15,23,42,0.35)] backdrop-blur-xl backdrop-saturate-150 transition-[opacity,transform] duration-[220ms] ease-[cubic-bezier(0.16,1,0.3,1)] data-[ending-style]:translate-y-3 data-[ending-style]:scale-[0.97] data-[ending-style]:opacity-0 data-[starting-style]:translate-y-3 data-[starting-style]:scale-[0.97] data-[starting-style]:opacity-0 supports-[backdrop-filter]:bg-white/80 dark:border-white/10 dark:bg-oai-gray-950/90 dark:shadow-[0_28px_80px_-20px_rgba(0,0,0,0.75)] dark:supports-[backdrop-filter]:bg-oai-gray-950/80 sm:max-h-[88vh]"
    >
        {/* Modal header — title + source link + close X */}
